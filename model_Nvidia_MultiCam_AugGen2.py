...
def Get3CarCameraImages(path, steering_center, HardBias=False, steering_HardBias=0):
    ...
    if (HardBias == True):
        ...
        steering_left = steering_center - steering_HardBias/2
        steering_right = steering_center + steering_HardBias/2
...
for offset in range(0, len(imagesCenter)):
    imgCenter = np.array(imagesCenter[offset])
    imgLeft = np.array(imagesLeft[offset])
    imgRight = np.array(imagesRight[offset])
    imgCenterFlip = np.fliplr(imgCenter)
    imgLeftFlip = np.fliplr(imgLeft)
    imgRightFlip = np.fliplr(imgRight)
    measCenter = np.array(measurementsCenter[offset])
    measLeft = np.array(measurementsLeft[offset])
    measRight = np.array(measurementsRight[offset])
    measCenterFlip = -measCenter
    measLeftFlip = -measLeft
    measRightFlip = -measRight
    X_train.append(imgCenter)
    X_train.append(imgLeft)
    X_train.append(imgRight)
    X_train.append(imgCenterFlip)
    X_train.append(imgLeftFlip)
    X_train.append(imgRightFlip)
    y_train.append(measCenter)
    y_train.append(measLeft)
    y_train.append(measRight)
    y_train.append(measCenterFlip)
    y_train.append(measLeftFlip)
    y_train.append(measRightFlip)
...
model.save('model_Nvidia_MultiCam_AugGenerator7.h5')
